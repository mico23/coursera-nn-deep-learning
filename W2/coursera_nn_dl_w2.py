...
def propagate(w, b, X, Y):
    ...
    j_1 = np.multiply(Y, np.log(A))             # do not confused with np.dot(Y, np.log(A))
    j_2 = np.multiply((1-Y), np.log(1-A))
    cost = (-1/m)*np.sum(j_1 + j_2)             # compute cost
    ### END CODE HERE ###
...
# Define the model of logistic function
